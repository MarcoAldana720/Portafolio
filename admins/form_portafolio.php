...
<body>
  ...
  <section class="content_form">
    <div class="content_left_2">
      <h2>Agregar nuevo portafolio</h2><br>
      <form action="add_portafolio.php" method="post" enctype="multipart/form-data">
        <label for="port_titulo">Titulo del proyecto:</label><br>
        <input type="text" id="port_titulo" name="port_titulo"><br>

        <label for="port_descripcion">Descripción:</label><br>
        <textarea type="text" name="port_descripcion" id="port_descripcion"></textarea><br>

        <label for="port_id_tecnologias">Tecnologías:</label><br>
        <select name="port_id_tecnologias[]" id="port_id_tecnologias" multiple multiselect-search="true" multiselect-select-all="true" multiselect-max-items="3" class="select_tag">
          <?php
            include '../conexion.php';
            $sql = "SELECT tec_id, tec_descripcion FROM tecnologias";
            $result = mysqli_query($conectar, $sql);
            while ($row = mysqli_fetch_assoc($result)) {
              echo "<option value='{$row['tec_id']}'>{$row['tec_descripcion']}</option>";
            }
          ?>
        </select>

        <label for="port_link">Link de producción:</label><br>
        <input type="text" id="port_link" name="port_link"><br>
